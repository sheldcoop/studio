...
export function TableOfContents({ subTopics }: TableOfContentsProps) {
    ...
    useEffect(() => {
        ...
        // A helper function to find and observe elements
        const observeElements = () => {
            const elements = subTopics
        ...
        // Small delay to ensure DOM is ready before we start observing
        const timeoutId = setTimeout(() => {
            observedElements = observeElements();
        }, 100);

        // Proper cleanup function
        return () => {
            clearTimeout(timeoutId);
            // Disconnect from all elements that were being observed
            if (observedElements.length > 0) {
                observedElements.forEach(el => observer.unobserve(el));
            }
            // Disconnect the observer entirely to prevent memory leaks
            observer.disconnect();
        };
    }, [subTopics]); // Dependency array is correct
