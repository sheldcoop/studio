...
def solve_decomposition(event):
    ...
    try:
        if "${operation}" == "lu":
            ...
            lu, piv = lu_factor(A)
            x = lu_solve((lu, piv), b)
            output = f"Solving Ax = b using LU Decomposition\\n\\n"
            output += f"Matrix A:\\n{A}\\n\\n"
            output += f"Vector b:\\n{b}\\n\\n"
            output += f"1. Decompose A into P, L, U (via lu_factor):\\n"
            output += f"   (Note: SciPy combines P and L for efficiency)\\n\\n"
            ...
            output = f"Solving Ax = b for the least-squares solution using QR Decomposition\\n\\n"
            output += f"Matrix A:\\n{A}\\n\\n"
            output += f"Vector b:\\n{b}\\n\\n"
            output += f"1. Decompose A into Q and R:\\n"
            output += f"   Q (Orthogonal Matrix):\\n{np.round(Q, 4)}\\n\\n"
            output += f"   R (Upper Triangular Matrix):\\n{np.round(R, 4)}\\n\\n"
            output += f"2. Solve Rx = Q.T * b:\\n"
            ...
            output = f"Solving Ax = b using Cholesky Decomposition\\n\\n"
            output += f"Matrix A (must be symmetric positive-definite):\\n{A}\\n\\n"
            output += f"Vector b:\\n{b}\\n\\n"
            output += f"1. Decompose A into L and L.T:\\n"
...
document.body.addEventListener('run-${operation}', create_proxy(solve_decomposition))
  ...
  return (
    <div>
        ...
        <pre id={outputId} className="mt-4 p-4 rounded-lg bg-muted/50 text-sm overflow-x-auto min-h-[200px]">
