 ...
 * This is a highly accurate method for approximating the gamma function for complex numbers.
...
/**
 * Calculates the Beta function B(alpha, beta).
 * @param alpha - The alpha parameter.
 * @param beta - The beta parameter.
 * @returns The value of B(alpha, beta).
 */
...
// --- Probability Mass Functions (PMFs) for Discrete Distributions ---
...
export const multinomialProbability = (n: number, x: number[], p: number[]): number => {
    ...
    let combinationsVal = factorial(n);
    let prob_product = 1;

    for (let i = 0; i < x.length; i++) {
        combinationsVal /= factorial(x[i]);
        prob_product *= Math.pow(p[i], x[i]);
    }
    
    return combinationsVal * prob_product;
};


// --- Probability Density Functions (PDFs) for Continuous Distributions ---
...
// --- Data Generation & Basic Statistics ---
