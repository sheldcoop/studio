...
export const getPathById = (id: string): LearningPath | undefined => {
  ...
  const pathModules = allModules.filter(m => m.parent === id).map(module => {
    ...
    if (module.id === 'prob-dist-discrete') {
        ...
        lessons = allTopics.filter(t => ['gamma-distribution', 'beta-distribution', 'exponential-distribution', 'cauchy-distribution', 'laplace-distribution', 'lognormal-distribution', 'normal-distribution', 'f-distribution'].includes(t.id));
    } else {
