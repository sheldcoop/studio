...
// --- Chart Components ---
const GoodnessOfFitChart = ({ generateData }: { generateData: () => void }) => {
...
const TestForIndependenceChart = ({ generateData }: { generateData: () => void }) => {
  const [chartData, setChartData] = React.useState<any[]>([]);
  ...
  return (
    <>
      ...
      <p className="mt-4 text-center text-sm text-muted-foreground">The test checks if the differences between observed and expected counts are significant.</p>
    </>
  );
};

const TestForHomogeneityChart = ({ generateData }: { generateData: () => void }) => {
  const [chartData, setChartData] = React.useState<any[]>([]);
  ...
  examples: [
    {
      ...
      ChartComponent: GoodnessOfFitChart as ComponentType<{ generateData: () => void }>,
    ...
    },
    {
      ...
      ChartComponent: TestForIndependenceChart as ComponentType<{ generateData: () => void }>,
      buttonText: 'Simulate New Data',
    },
    {
      id: 'homogeneity',
      ...
      ChartComponent: TestForHomogeneityChart as ComponentType<{ generateData: () => void }>,
      buttonText: 'Simulate New Survey Data',
    },
  ],
};
