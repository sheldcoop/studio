...
// --- Chart Components ---
const OneWayAnovaChart = ({ generateData }: { generateData: () => void }) => {
...
const TwoWayAnovaChart = ({ generateData }: { generateData: () => void }) => {
  const [chartData, setChartData] = React.useState<any[]>([]);
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      <p className="mt-4 text-center text-sm text-muted-foreground">Non-parallel lines suggest an interaction effect.</p>
    </>
  );
};

const RepeatedMeasuresAnovaChart = ({ generateData }: { generateData: () => void }) => {
  const [chartData, setChartData] = React.useState<any[]>([]);
  React.useEffect(() => {
  ...
  examples: [
    {
      ...
      ChartComponent: OneWayAnovaChart as ComponentType<{ generateData: () => void }>,
      buttonText: 'Simulate New 50-Month Period',
    },
    {
      id: 'two-way',
      title: 'Two-Way ANOVA',
      ...
      ChartComponent: TwoWayAnovaChart as ComponentType<{ generateData: () => void }>,
    ...
    },
    {
      ...
      ChartComponent: RepeatedMeasuresAnovaChart as ComponentType<{ generateData: () => void }>,
      buttonText: 'Simulate New Portfolio Journey',
    },
  ],
};
