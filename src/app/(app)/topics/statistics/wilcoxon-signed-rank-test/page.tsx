...
const WilcoxonSignedRankChart = () => {
  ...
  const generateData = () => {
    ...
    const afterData = beforeData.map(
      (d) => d + (Math.random() * 0.8 - 0.4) // Apply a change, can be positive or negative
    );
    setChartData(
      ...
      <div className="flex-grow">
        <ChartContainer config={wilcoxonChartConfig} className="h-full w-full">
            <RechartsLineChart accessibilityLayer data={chartData} margin={{ top: 20, right: 30, bottom: 20, left: 30 }}>
                ...
                <YAxis unit="%" name="Max Drawdown" />
                <Tooltip content={<ChartTooltipContent />} />
                <Legend formatter={(value) => wilcoxonChartConfig[value as keyof typeof wilcoxonChartConfig]?.label || value} />
                <Line type="monotone" dataKey="Before_Risk_Model" stroke="var(--color-Before_Risk_Model)" strokeWidth={2} />
                <Line type="monotone" dataKey="After_Risk_Model" stroke="var(--color-After_Risk_Model)" strokeWidth={2} />
            </RechartsLineChart>
        </ChartContainer>
      </div>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                <p className="text-muted-foreground">
                  This test is the non-parametric version of the Paired T-Test. It's designed for comparing two related measurements from the same subject. Instead of using raw data, it ranks the absolute differences between pairs to see if there's a significant change.
                </p>
              </div>
              <div>
                <h3 className="mb-1 font-semibold text-primary">
