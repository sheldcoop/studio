...
const createHistogram = (data: number[], binSize: number, min: number, max: number) => {
    ...
    if (!isFinite(bins)) return { labels: [], counts: [] };
  ...
  return (
    <div className="flex h-fit w-full flex-col">
      <div className="relative mx-auto flex-grow w-full">
        <ChartContainer config={mannWhitneyChartConfig} className="h-[350px] w-full">
          <RechartsBarChart accessibilityLayer data={chartData} barCategoryGap="0%" margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <Bar dataKey="Algo_A" fill="var(--color-Algo_A)" opacity={0.7} />
            <Bar dataKey="Algo_B" fill="var(--color-Algo_B)" opacity={0.7} />
          </RechartsBarChart>
        </ChartContainer>
      </div>
