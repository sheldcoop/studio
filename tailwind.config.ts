import type {Config} from 'tailwindcss';

const config: Config = {
  darkMode: 'class',
  content: [
    './src/pages/**/*.{js,ts,jsx,tsx,mdx}',
    './src/components/**/*.{js,ts,jsx,tsx,mdx}',
    './src/app/**/*.{js,ts,jsx,tsx,mdx}',
  ],
  theme: {
    extend: {
      fontFamily: {
        body: ['var(--font-body)', 'sans-serif'],
        headline: ['var(--font-headline)', 'sans-serif'],
      },
      colors: {
        background: 'hsl(var(--background))',
        foreground: 'hsl(var(--foreground))',
        card: {
          DEFAULT: 'hsl(var(--card))',
          foreground: 'hsl(var(--card-foreground))',
        },
        popover: {
          DEFAULT: 'hsl(var(--popover))',
          foreground: 'hsl(var(--popover-foreground))',
        },
        primary: {
          DEFAULT: 'hsl(var(--primary))',
          foreground: 'hsl(var(--primary-foreground))',
        },
        secondary: {
          DEFAULT: 'hsl(var(--secondary))',
          foreground: 'hsl(var(--secondary-foreground))',
        },
        muted: {
          DEFAULT: 'hsl(var(--muted))',
          foreground: 'hsl(var(--muted-foreground))',
        },
        accent: {
          DEFAULT: 'hsl(var(--accent))',
          foreground: 'hsl(var(--accent-foreground))',
        },
        destructive: {
          DEFAULT: 'hsl(var(--destructive))',
          foreground: 'hsl(var(--destructive-foreground))',
        },
        border: 'hsl(var(--border))',
        input: 'hsl(var(--input))',
        ring: 'hsl(var(--ring))',
        chart: {
          '1': 'hsl(var(--chart-1))',
          '2': 'hsl(var(--chart-2))',
          '3': 'hsl(var(--chart-3))',
          '4': 'hsl(var(--chart-4))',
          '5': 'hsl(var(--chart-5))',
        },
        sidebar: {
          DEFAULT: 'hsl(var(--sidebar-background))',
          foreground: 'hsl(var(--sidebar-foreground))',
          primary: 'hsl(var(--sidebar-primary))',
          'primary-foreground': 'hsl(var(--sidebar-primary-foreground))',
          accent: 'hsl(var(--sidebar-accent))',
          'accent-foreground': 'hsl(var(--sidebar-accent-foreground))',
          border: 'hsl(var(--sidebar-border))',
          ring: 'hsl(var(--sidebar-ring))',
        },
      },
      borderRadius: {
        lg: 'var(--radius)',
        md: 'calc(var(--radius) - 2px)',
        sm: 'calc(var(--radius) - 4px)',
      },
      keyframes: {
        'accordion-down': {
          from: {
            height: '0',
          },
          to: {
            height: 'var(--radix-accordion-content-height)',
          },
        },
        'accordion-up': {
          from: {
            height: 'var(--radix-accordion-content-height)',
          },
          to: {
            height: '0',
          },
        },
        blink: {
          '50%': { 'border-color': 'transparent' },
        }
      },
      animation: {
        'accordion-down': 'accordion-down 0.2s ease-out',
        'accordion-up': 'accordion-up 0.2s ease-out',
        blink: 'blink 1s step-end infinite',
      },
    },
  },
  plugins: [
    require('tailwindcss-animate'),
    function ({ addBase }: { addBase: (styles: any) => void }) {
      addBase({
        ':root': {
          '--background': '210 40% 98%',
          '--foreground': '222.2 84% 4.9%',
          '--card': '210 40% 98%',
          '--card-foreground': '222.2 84% 4.9%',
          '--popover': '210 40% 98%',
          '--popover-foreground': '222.2 84% 4.9%',
          '--primary': '222.2 47.4% 11.2%',
          '--primary-foreground': '210 40% 98%',
          '--secondary': '210 40% 96.1%',
          '--secondary-foreground': '222.2 47.4% 11.2%',
          '--muted': '210 40% 96.1%',
          '--muted-foreground': '215.4 16.3% 46.9%',
          '--accent': '210 40% 96.1%',
          '--accent-foreground': '222.2 47.4% 11.2%',
          '--destructive': '0 84.2% 60.2%',
          '--destructive-foreground': '210 40% 98%',
          '--border': '214.3 31.8% 91.4%',
          '--input': '214.3 31.8% 91.4%',
          '--ring': '222.2 47.4% 11.2%',
          '--chart-1': '222.2 47.4% 11.2%',
          '--chart-2': '222.2 47.4% 30%',
          '--chart-3': '222.2 47.4% 50%',
          '--chart-4': '222.2 47.4% 70%',
          '--chart-5': '222.2 47.4% 90%',
          '--animation-primary-color': 'hsl(222.2, 47.4%, 11.2%)',
          '--animation-opacity': '0.85',
          '--prism-background': '#f5f2f0',
          '--prism-foreground': '#393A34',
          '--prism-comment': '#999',
          '--prism-punctuation': '#999',
          '--prism-property': '#905',
          '--prism-selector': '#690',
          '--prism-operator': '#a67f59',
          '--prism-keyword': '#07a',
          '--prism-function': '#DD4A68',
          '--prism-variable': '#e90',
        },
        '.dark': {
          '--background': '222 89% 6%',
          '--foreground': '210 40% 98%',
          '--card': '222 72% 9%',
          '--card-foreground': '210 40% 98%',
          '--popover': '222 89% 6%',
          '--popover-foreground': '210 40% 98%',
          '--primary': '173 98% 46%',
          '--primary-foreground': '173 100% 10%',
          '--secondary': '217 33% 17%',
          '--secondary-foreground': '210 40% 98%',
          '--muted': '217 33% 17%',
          '--muted-foreground': '215 20% 65%',
          '--accent': '217 33% 17%',
          '--accent-foreground': '210 40% 98%',
          '--destructive': '0 63% 31%',
          '--destructive-foreground': '210 40% 98%',
          '--border': '217 33% 17%',
          '--input': '217 33% 17%',
          '--ring': '173 98% 46%',
          '--chart-1': '173 98% 46%',
          '--chart-2': '259 44% 80%',
          '--chart-3': '0 72.2% 50.6%',
          '--chart-4': '43.2 95.5% 53.1%',
          '--chart-5': '270 95.5% 53.1%',
          '--animation-primary-color': 'hsl(173, 98%, 46%)',
          '--animation-opacity': '0.85',
          '--prism-background': 'hsl(222, 72%, 12%)',
          '--prism-foreground': '#ccc',
          '--prism-comment': '#787878',
          '--prism-punctuation': '#ccc',
          '--prism-property': '#9876aa',
          '--prism-selector': '#6a8759',
          '--prism-operator': '#fc9',
          '--prism-keyword': '#cda869',
          '--prism-function': '#ffc66d',
          '--prism-variable': '#a9b7c6',
        },
        '.slate': {
          '--background': '223 24% 10%',
          '--foreground': '220 13% 91%',
          '--card': '223 24% 10%',
          '--card-foreground': '220 13% 91%',
          '--popover': '223 24% 10%',
          '--popover-foreground': '220 13% 91%',
          '--primary': '38 92% 50%',
          '--primary-foreground': '38 100% 10%',
          '--secondary': '226 24% 16%',
          '--secondary-foreground': '220 13% 91%',
          '--muted': '226 24% 16%',
          '--muted-foreground': '226 12% 46%',
          '--accent': '226 24% 16%',
          '--accent-foreground': '38 92% 50%',
          '--destructive': '0 63% 31%',
          '--destructive-foreground': '220 13% 91%',
          '--border': '226 24% 20%',
          '--input': '226 24% 20%',
          '--ring': '38 92% 50%',
          '--chart-1': '38 92% 50%',
          '--chart-2': '259 44% 80%',
          '--chart-3': '0 72.2% 50.6%',
          '--chart-4': '43.2 95.5% 53.1%',
          '--chart-5': '270 95.5% 53.1%',
          '--animation-primary-color': 'hsl(38, 92%, 50%)',
          '--animation-opacity': '0.85',
          '--prism-background': 'hsl(223, 24%, 15%)',
          '--prism-foreground': '#a9b7c6',
          '--prism-comment': '#808080',
          '--prism-punctuation': '#a9b7c6',
          '--prism-property': '#ffc66d',
          '--prism-selector': '#a5c261',
          '--prism-operator': '#a9b7c6',
          '--prism-keyword': '#cc7832',
          '--prism-function': '#ffc66d',
          '--prism-variable': '#a9b7c6',
        },
        '.nocturne': {
          '--background': '220 10% 12%',
          '--foreground': '40 20% 85%',
          '--card': '220 10% 12%',
          '--card-foreground': '40 20% 85%',
          '--popover': '220 10% 12%',
          '--popover-foreground': '40 20% 85%',
          '--primary': '40 20% 85%',
          '--primary-foreground': '220 10% 12%',
          '--secondary': '220 10% 20%',
          '--secondary-foreground': '40 20% 85%',
          '--muted': '220 10% 20%',
          '--muted-foreground': '40 10% 55%',
          '--accent': '220 10% 20%',
          '--accent-foreground': '40 20% 85%',
          '--destructive': '0 63% 31%',
          '--destructive-foreground': '40 20% 85%',
          '--border': '220 10% 25%',
          '--input': '220 10% 25%',
          '--ring': '40 20% 85%',
          '--chart-1': '40 20% 85%',
          '--chart-2': '40 20% 65%',
          '--chart-3': '40 20% 55%',
          '--chart-4': '40 20% 45%',
          '--chart-5': '40 20% 35%',
          '--animation-primary-color': 'hsl(40, 20%, 85%)',
          '--animation-opacity': '0.7',
          '--prism-background': 'hsl(220, 10%, 18%)',
          '--prism-foreground': '#d4d4d4',
          '--prism-comment': '#6a9955',
          '--prism-punctuation': '#d4d4d4',
          '--prism-property': '#569cd6',
          '--prism-selector': '#ce9178',
          '--prism-operator': '#d4d4d4',
          '--prism-keyword': '#c586c0',
          '--prism-function': '#dcdcaa',
          '--prism-variable': '#9cdcfe',
        },
        '.quant': {
          '--background': '220 30% 7%',
          '--foreground': '200 15% 92%',
          '--card': '220 25% 10%',
          '--card-foreground': '200 15% 92%',
          '--popover': '220 30% 7%',
          '--popover-foreground': '200 15% 92%',
          '--primary': '180 85% 50%',
          '--primary-foreground': '220 30% 7%',
          '--secondary': '220 20% 15%',
          '--secondary-foreground': '200 15% 92%',
          '--muted': '220 20% 15%',
          '--muted-foreground': '200 10% 55%',
          '--accent': '280 70% 60%',
          '--accent-foreground': '200 15% 92%',
          '--destructive': '0 63% 31%',
          '--destructive-foreground': '200 15% 92%',
          '--border': '220 20% 18%',
          '--input': '220 20% 18%',
          '--ring': '180 85% 50%',
          '--chart-1': '180 85% 50%',
          '--chart-2': '280 70% 60%',
          '--chart-3': '200 80% 55%',
          '--chart-4': '160 75% 55%',
          '--chart-5': '260 65% 60%',
          '--animation-primary-color': 'hsl(180, 85%, 50%)',
          '--animation-opacity': '0.9',
          '--prism-background': 'hsl(220, 25%, 12%)',
          '--prism-foreground': '#80b3ff',
          '--prism-comment': '#6272a4',
          '--prism-punctuation': '#f8f8f2',
          '--prism-property': '#ff79c6',
          '--prism-selector': '#50fa7b',
          '--prism-operator': '#ff79c6',
          '--prism-keyword': '#ff79c6',
          '--prism-function': '#50fa7b',
          '--prism-variable': '#8be9fd',
        },
      });
    },
  ],
};

export default config;
